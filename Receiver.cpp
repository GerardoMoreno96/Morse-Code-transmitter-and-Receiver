...
int contador3 = 0;
int foresaso = 0;
int array1[200], array2[450], array3[450], array4[450], array5[450], array6[450], array7[450], array8[450], array9[450], array10[450], array11[450], array12[450];
...
public:
	...
	DATO BuscarLetra(const std::string mors);
...
template<class DATO>
DATO ABB<DATO>::BuscarLetra(const std::string mors)
{
	actual = raiz;
	int x = 0;
	while (x < mors.length())
...
void principal()
{
	...
	std::string palabra;
	std::string leido;
	for (int i = 0; i < contadorarray1; i++)
	{
		leido = array1[i];
		palabra = palabra + leido;
	}

	char letra = Arbolm.BuscarLetra(palabra);
	system("pause");
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->serialPort1->DataReceived += gcnew System::IO::Ports::SerialDataReceivedEventHandler(this, &Enigma::serialPort1_DataReceived);
			// 
			// Enigma
			// 
			this->AutoScaleDimensions = System::Drawing::SizeF(6, 13);
			this->AutoScaleMode = System::Windows::Forms::AutoScaleMode::Font;
			this->ClientSize = System::Drawing::Size(967, 397);
			this->Name = L"Enigma";
			this->Text = L"Enigma";
...
#pragma endregion
	...
	private: System::Void serialPort1_DataReceived(System::Object^  sender, System::IO::Ports::SerialDataReceivedEventArgs^  e) {
		...
		char convertido;
		if (contadorx < 80) {
			array1[contadorarray] = serialPort1->ReadLine()->ToCharArray()[0];
			contadorarray1++;
			if (contadorx >= 70) {
				reducirArreglo();
			}
	...
	private: void reducirArreglo() {
		MessageBox::Show("REDUCIR");
		char anterior = 'z';
		int contador = 0;
		for (int i = 0; i < contadorarray1; i++) {
			if (array1[i] != anterior) {
				if (array1[i] == '.' || array1[i] == '-') {
					array1[contador] = array1[i];
					contador++;
				}
				anterior = array1[i];
			}
		...
		for (int g = 0; g < contador; g++) {
			MessageBox::Show(array1[g].ToString());
		}
		contadorarray1 = contador;
		principal();
	}
	};
}
